...
describe('findSuspiciousInProductGroup', () => {
  it('flags unit and qty OCR in Menabrea-like series', () => {
    const group = [
      { id: 'a', fornitore_id: fid, prodotto: 'Beer', prezzo: 35.66, data_prezzo: '2026-01-01' },
      { id: 'b', fornitore_id: fid, prodotto: 'Beer', prezzo: 1.48, data_prezzo: '2026-02-01' },
      { id: 'c', fornitore_id: fid, prodotto: 'Beer', prezzo: 5.02, data_prezzo: '2026-02-15' },
      { id: 'd', fornitore_id: fid, prodotto: 'Beer', prezzo: 36.04, data_prezzo: '2026-03-01' },
    ]
    expect(findSuspiciousInProductGroup(group).map((r) => r.id).sort()).toEqual(['b', 'c'])
  })
...
describe('findSuspiciousListinoRows', () => {
  it('flags unit price when case-price cluster dominates', () => {
    const rows = [
    ...
    expect(findSuspiciousListinoRows(rows).map((r) => r.id)).toEqual(['1'])
  })
})
