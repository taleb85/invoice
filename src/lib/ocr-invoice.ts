...
Return ONLY valid JSON — no markdown, no explanation:
{
  ...
  "promessa_invio_documento": false,
  "totale_iva_inclusa": "The gross total amount — return the RAW string exactly as it appears (e.g. '1.234,56' or '£1,234.56' or '1234.56') so the caller can detect the numeric format. For **bolla_ddt** only: use null unless a clear invoice-style VAT grand total is printed (not line Value columns).",
...
- totale_iva_inclusa: return the raw amount string EXACTLY as printed (including any currency symbol and separators). Do NOT convert to a number. For **bolla_ddt**, prefer **null** here unless the document is clearly a tax invoice with a VAT grand total.
