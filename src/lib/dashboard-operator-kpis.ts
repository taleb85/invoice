...
/** Tag BCP‑47 per etichette mese del grafico dashboard (serie aggregata KPI). */
function chartLocaleTag(localeCode: Locale): string {
  ...
  return m[localeCode] ?? 'en-GB'
}

function monthKeyUtcFromYMD(dateStr: string): string {
  return dateStr.slice(0, 7)
}

function utcDayBefore(exclusiveYYYYMMDD: string): string {
  const d = new Date(`${exclusiveYYYYMMDD}T12:00:00Z`)
  d.setUTCDate(d.getUTCDate() - 1)
  return d.toISOString().slice(0, 10)!
}

/** Punto serie “andamento mensile” sulla dashboard Aurora (FY o ultimi mesi). */
export type DashboardHomeMonthlyPoint = {
  key: string
  label: string
  spend: number
  bolle: number
...
const DASHBOARD_HOME_TREND_ROWS = 5000 as const

/**
 * Serie mensile bolle/fatture/importo nello stesso ambito KPI operatore (`fornitoreIds` + FY).
 */
export async function fetchDashboardHomeMonthlyTrend(
  supabase: SupabaseClient,
  fornitoreIds: string[] | null,
  bounds: FiscalPgBounds | null,
  localeCode: Locale,
  maxMonths = 12,
): Promise<DashboardHomeMonthlyPoint[]> {
  const lc = chartLocaleTag(localeCode)
  let dateFromStr: string
  let dateToInclusiveStr: string
  let monthKeysMax = maxMonths

  if (bounds) {
    dateFromStr = bounds.dateFrom
    const fyEndInclusive = utcDayBefore(bounds.dateToExclusive)
    const todayInclusive = new Date().toISOString().slice(0, 10)!
    dateToInclusiveStr = fyEndInclusive < todayInclusive ? fyEndInclusive : todayInclusive
  } else {
    const now = new Date()
    const start = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - 5, 1, 12, 0, 0))
    dateFromStr = start.toISOString().slice(0, 10)!
    dateToInclusiveStr = now.toISOString().slice(0, 10)!
    monthKeysMax = 6
  }

  if (dateFromStr > dateToInclusiveStr) return []

  const aggKeys: string[] = []
  let cursor = new Date(`${dateFromStr.slice(0, 7)}-01T12:00:00Z`)
  const endMonth = new Date(`${dateToInclusiveStr.slice(0, 7)}-01T12:00:00Z`)
  while (aggKeys.length < monthKeysMax && cursor <= endMonth) {
    aggKeys.push(cursor.toISOString().slice(0, 7)!)
    cursor = new Date(Date.UTC(cursor.getUTCFullYear(), cursor.getUTCMonth() + 1, 1, 12, 0, 0))
  }
  if (!aggKeys.length) return []

  let bolleQ = supabase.from('bolle').select('data').gte('data', dateFromStr).lte('data', dateToInclusiveStr).limit(DASHBOARD_HOME_TREND_ROWS)
  let fattureQ = supabase
    .from('fatture')
    .select('data, importo')
    .gte('data', dateFromStr)
    .lte('data', dateToInclusiveStr)
    .limit(DASHBOARD_HOME_TREND_ROWS)

  const fid = fornitoreIds?.length ? fornitoreIds : null
  if (fid) {
    bolleQ = bolleQ.in('fornitore_id', fid)
    fattureQ = fattureQ.in('fornitore_id', fid)
  }

  const [bolleRes, fattRes] = await Promise.all([bolleQ, fattureQ])
  if (bolleRes.error) console.error('[fetchDashboardHomeMonthlyTrend] bolle', bolleRes.error.message)
  if (fattRes.error) console.error('[fetchDashboardHomeMonthlyTrend] fatture', fattRes.error.message)

  const spendMap = new Map<string, number>()
  const bolleMap = new Map<string, number>()
  const fattMap = new Map<string, number>()

  for (const r of bolleRes.data ?? []) {
    const data = typeof (r as { data?: unknown }).data === 'string' ? ((r as { data: string }).data as string) : ''
    if (!data) continue
    const mk = monthKeyUtcFromYMD(data)
    bolleMap.set(mk, (bolleMap.get(mk) ?? 0) + 1)
  }
  for (const r of fattRes.data ?? []) {
    const row = r as { data?: string | null; importo?: number | null }
    if (!row.data) continue
    const mk = monthKeyUtcFromYMD(row.data)
    spendMap.set(mk, (spendMap.get(mk) ?? 0) + (Number(row.importo) || 0))
    fattMap.set(mk, (fattMap.get(mk) ?? 0) + 1)
  }
  ...
  return aggKeys.map((key) => {
    ...
    return {
      key,
      label,
      spend: Math.round((spendMap.get(key) ?? 0) * 100) / 100,
      bolle: bolleMap.get(key) ?? 0,
      fatture: fattMap.get(key) ?? 0,
    }
  })
}
