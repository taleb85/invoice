...
type Translations = {
  ...
  dashboard: {
    ...
    /** Card Aurora: quota bolle non «in attesa» sul totale nel periodo KPI. */
    homeSmartPairTitle: string
    /** `{pct}` = percentuale intera (bolle completed vs totale). */
    homeSmartPairMatchedLine: string
    /** Carico stimato su revisione / estratti / duplicati (solo etichetta UI). */
    ...
    /** Fornitore OCR/email errato o documento letto male: guida nella modale duplicati */
    ...
    /** Tile «Documenti da revisionare»: nell’FY selezionato conta duplicati, «da associare» in posta, anomalie; il reprocess dashboard invia `fiscal_*` per allinearci. */
    kpiDocumentiDaRevisionareTitle: string
    kpiDocumentiDaRevisionareSub: string
    /**
     * Tile vetro KPI documenti da revisionare: spiega differenza fra totale grande e subset «da associare».
     * Segnaposti: `{fyLabel}` `{da_associare}` `{total}`
     */
    ...
    /** Portale globale — stato tecnico sede */
  ...
  log: {
    ...
    /** Toast elaborazione: segnaposto `{auto}` salvati, `{rev}` in revisione */
    activityProcessToastDetail: string
    activityQueueEmptyCelebrate: string
    /** Intestazione colonna azioni su righe «fornitore da aggiungere» */
    activityLogRowActions: string
    activityIgnoreSenderDoneToast: string
    /** Toast dopo nuovo fornitore: segnaposto `{n}` documenti riprocessati */
    activitySupplierAddedReprocessedToast: string
    activityDocDiscardedToast: string
    /** Toast AI Inbox: scarto automatico documenti tipo «altro», alta confidenza. Segnaposto `{n}`. */
    activityAiInboxAutoDiscardedToast: string
    /** AI Inbox Gemini: cronologia locale ultime analisi — double-check visivo */
  ...
  sedi: {
    ...
    /** Password field on branch IMAP form — omit PATCH field when left blank */
    ...
    /** Etichetta breve pulsante copia UUID sede */
    copyTechnicalIdButton: string
    /** Lista sedi: switch veloce conservazione allegati */
    ...
    /** Elenco operatori: modifica nome; PIN ed eliminazione per ruolo operatore */
    ...
    /** Badge compatto ruolo sede tecnico (`admin_tecnico`) — distinto da `adminSedeRoleShort`. */
    adminTecnicoRoleShort: string
    valutaFuso: string
    ...
    /** DB senza colonne retention / schema cache PostgREST non allineato */
  ...
  appStrings: {
    ...
    /** Extra line on approvals page — master admin, all branches */
    approvazioni_masterNetworkHint: string
    /** Analytics page subtitle */
    analyticsPageSub: string
    /** Pagina analytics senza sede operativa (master globale) */
...
const it: Translations = {
  ...
  login: {
    ...
    adminOnlyEmail:
      'Solo l\'Admin Master (ruolo «admin» sul profilo) può entrare qui. Responsabili di azienda, amministratori tecnici («admin_tecnico») ed operatori: usa «Accesso azienda». Per ottenere il ruolo master contatta chi gestisce gli account.',
  ...
  sedi: {
    ...
    adminTecnicoRole: 'Amministratore tecnico',
