...
export default function GmailAuditReadyBadge({ fornitoreNome }: GmailAuditReadyBadgeProps) {
  ...
  const checkGmailStatus = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Show badge only if Gmail is not fully set up (but could be)
        // Don't show if already connected (user knows it works)
        if (!data.connected) {
          setShow(true)
        }
      }
  ...
  const scrollToAudit = () => {
    // Scroll to the price history scanner section
    const historySection = document.getElementById('rekki-price-history-scanner')
    if (historySection) {
      historySection.scrollIntoView({ behavior: 'smooth', block: 'start' })
      
      // Optionally trigger a subtle highlight effect
      historySection.classList.add('ring-2', 'ring-cyan-500/40')
      setTimeout(() => {
        historySection.classList.remove('ring-2', 'ring-cyan-500/40')
      }, 2000)
    }
  }

  if (dismissed || !show || !gmailStatus) {
    return null
  }
  ...
  return (
    <div className="mb-5 rounded-xl border border-gradient-to-r from-cyan-500/30 to-blue-500/30 bg-gradient-to-br from-cyan-500/10 via-blue-500/10 to-purple-500/10 p-4 shadow-lg backdrop-blur-sm">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1 text-xs leading-relaxed text-cyan-200/80">
            {gmailStatus.configured ? (
              <>
                Gmail API è configurato! Connetti il tuo account per attivare lo scanner automatico e
                recuperare potenziali rimborsi su <span className="font-semibold text-cyan-100">{fornitoreNome}</span>.
              </>
            ) : (
              <>
                Configura Gmail (2 minuti) per analizzare automaticamente le email di{' '}
                <span className="font-semibold text-cyan-100">{fornitoreNome}</span> e identificare
                overcharges non autorizzati.
              </>
            )}
          </p>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {gmailStatus.configured ? 'Connetti e Scansiona' : 'Configura Ora'}
            </button>
      ...
      {/* Progress Indicator (if configured but not connected) */}
      ...
      {!gmailStatus.configured && (
        <div className="mt-3 grid grid-cols-3 gap-2">
          ...
          <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 px-2 py-1.5 text-center">
            <p className="text-[10px] font-bold text-blue-200">Controllo Prezzi</p>
            <p className="text-[9px] text-blue-300/60">Auto anomalie</p>
          </div>
          <div className="rounded-lg border border-purple-500/20 bg-purple-500/5 px-2 py-1.5 text-center">
            ...
            <p className="text-[9px] text-purple-300/60">Storico 2 anni</p>
          </div>
        </div>
      )}
    </div>
