  ...
  return (
    ...
    >
        <nav className="app-shell-rail-panel relative z-0 flex min-h-0 flex-1 flex-col text-app-fg">
          ...
          {/* ── Admin: Sede Switcher + Gestisci sedi ── */}
          {isMasterAdmin && (
            <>
          ...
          {/* Gestisci sedi — voce principale dopo il switcher */}
          <Link
            href="/sedi"
            onClick={onClose}
        ...
        <div className="app-shell-rail-panel relative z-0 mt-auto flex shrink-0 flex-col border-t border-app-line-22 -mx-2.5 px-2.5 py-1.5 text-app-fg lg:-mx-3 lg:px-3">
            ...
            <div className="mt-1.5 space-y-0.5 border-t border-app-line-18 pt-1.5">
              ...
              <Link
                ...
                className={`flex w-full items-center gap-2 rounded-md px-2 py-1 text-xs font-semibold transition-colors ${
                  pathname?.startsWith('/strumenti') ? 'text-app-fg bg-app-line-10' : 'text-white/55 hover:bg-app-line-10 hover:text-app-fg'
                }`}
              >
          ...
          <div className={`flex items-center gap-0.5 pt-0.5 ${footerOpen ? 'hidden' : ''}`}>
            ...
            <Link
              href="/strumenti/centro-operazioni"
              title={t.strumentiCentroOperazioni.pageTitle}
              className={`flex h-7 w-7 items-center justify-center rounded-md transition-colors hover:bg-app-line-10 ${pathname?.startsWith('/strumenti') ? 'text-cyan-300' : 'text-white/40 hover:text-app-fg'}`}
            >
