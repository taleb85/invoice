...
export default function Sidebar({ onClose }: SidebarProps) {
  ...
  const [masterOperationalNavOpen, setMasterOperationalNavOpen] = useState(true)
  ...
  /** Master: voci operative sotto dashboard (Analitiche, …) — nel cassetto sotto l’elenco sedi. */
  const flatNavRestItems = navItems
    .slice(1)
    .filter(
      (item) =>
  ...
  const masterGestisciSediRailActive = pathname === '/strumenti/sedi' || pathname.startsWith('/sedi/')
  ...
  const railDrawerNavLink = (isActive: boolean) =>
    `flex min-w-0 items-center gap-2 rounded-md px-2 py-1 text-left text-xs font-semibold transition-colors touch-manipulation ${
      isActive
        ...
        <nav className="app-shell-rail-panel relative z-0 flex min-h-0 flex-1 flex-col text-app-fg">
          ...
          {isMasterAdmin && (
            ...
            <div className="bg-transparent">
              ...
              {(branchesOpen || flatNavRestItems.length > 0 || activeSede) && (
                <div className="app-shell-rail-panel ml-3 mt-0.5 min-w-0">
                  ...
                  {!branchesOpen && activeSede ? (
                    (() => {
                      ...
                      const nome =
                        (s?.nome?.trim() || getAssociatedSedeNome(me, getCookie).trim() || activeSede)
                      return (
                        <div className="space-y-0.5">
                          <div className="flex items-center">
                            <button
                              ...
                              title={t.ui.sidebarSedeActive.replace('{name}', nome)}
          ...
          {isAdminSede &&
            ...
            <Link href="/strumenti/sedi" onClick={onClose} className={`hidden ${navLink(pathname === '/strumenti/sedi')}`}>
        ...
        <div className="app-shell-rail-panel relative z-20 mt-auto flex shrink-0 flex-col border-t border-app-line-22 -mx-2.5 px-2.5 py-1.5 text-app-fg lg:-mx-3 lg:px-3">
          ...
          {footerOpen && (
            <div className="mt-1.5 space-y-0.5 border-t border-app-line-18 pt-1.5">
              ...
              <Link
                ...
                className={`hidden flex w-full items-center gap-2 rounded-md px-2 py-1 text-xs font-semibold transition-colors ${pathname === '/strumenti/impostazioni' ? 'text-app-fg bg-app-line-10' : 'text-app-fg-subtle hover:bg-app-line-10 hover:text-app-fg'}`}
              >
          ...
          <div className={`flex items-center gap-0.5 pt-0.5 ${footerOpen ? 'hidden' : ''}`}>
            ...
            <Link
              ...
              className="hidden flex h-7 w-7 items-center justify-center rounded-md transition-colors hover:bg-app-line-10 text-app-fg-subtle hover:text-app-fg"
            >
