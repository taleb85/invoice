...
export default function DashboardKpiListSheet({
  ...
  return (
    <>
      <button
        type="button"
        className={`app-card block w-full cursor-pointer touch-manipulation text-left transition-all hover:border-cyan-500/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/40 ${
        ...
        ? createPortal(
            <div className="fixed inset-0 z-[210] flex items-center justify-center p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
              ...
              <div
                ...
                className="app-card relative z-[211] mx-auto flex w-full max-w-lg max-h-[min(78vh,32rem)] flex-col overflow-hidden rounded-xl border-slate-700/50 shadow-2xl shadow-black/50"
              >
                <div className="app-card-bar" aria-hidden />
                {recentCardChrome ? (
                  <div className="flex items-center justify-between gap-3 border-b border-slate-700/60 px-5 py-3.5">
                    <h2 id={titleId} className="min-w-0 text-sm font-semibold text-slate-100">
                      {dialogTitle}
                    </h2>
                    <div className="flex shrink-0 items-center gap-2">
                      <Link
                        href={dialogViewAll}
                        onClick={close}
                        className="text-xs font-semibold text-cyan-400 transition-colors hover:text-cyan-300 hover:underline"
                      >
                        {t.dashboard.viewAll}
                      </Link>
                      <button
                ...
                <div
                  className={
                    recentCardChrome
                      ? 'min-h-0 flex-1 overflow-y-auto overscroll-contain divide-y divide-slate-800/80 pb-[max(0.75rem,env(safe-area-inset-bottom))]'
                ...
                {!recentCardChrome ? (
                  <div className="border-t border-slate-700/60 px-4 py-3 sm:px-5">
                    <Link
                      href={dialogViewAll}
                      onClick={close}
                      className="block text-center text-sm font-semibold text-cyan-400 transition-colors hover:text-cyan-300"
                    >
                      {t.dashboard.viewAll}
                    </Link>
                  </div>
