...
export default function NotificationBell({
  ...
  const buttonClass =
    variant === 'inline'
      ? `relative inline-flex h-8 min-w-8 shrink-0 items-center justify-center overflow-visible rounded-lg border border-slate-700/80 bg-slate-800/90 px-1 text-slate-200 transition-colors hover:border-slate-600 hover:bg-slate-800 touch-manipulation`
  ...
  const inner = (
    <div className={`relative ${variant === 'fab' ? '' : ''}`} ref={wrapRef}>
      <button
        ...
        onClick={toggle}
      >
        <Bell className={variant === 'fab' ? 'h-6 w-6' : 'h-4 w-4'} aria-hidden />
        {showBadge ? (
          <span
