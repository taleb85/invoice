...
import type { EmailActivityLogReprocess } from '@/components/email-activity-log-reprocess'
import type { EmailActivityProcOutcomeCode, EmailActivityRowElab } from '@/components/email-activity-log-reprocess'
import {
...
function labelForOutcome(code: EmailActivityProcOutcomeCode, detail: string | undefined, L: ProcLabels): string {
  const d = detail?.trim()
  switch (code) {
    case 'processed_auto':
...
function ElabCell({ row, elab, procLabels }: { row: EmailActivityRow; elab: EmailActivityRowElab; procLabels: ProcLabels }) {
  const id = docId(row)
  if (!id) {
...
function shouldShowElabInline(row: EmailActivityRow, elab: EmailActivityRowElab): boolean {
...
export function EmailActivityLogPanel({
  ...
  reprocess: EmailActivityLogReprocess
  /** Toolbar riepilogo+Elabora resa fuori dalla card (fascia sticky su `/log`). */
  hideToolbar?: boolean
}) {
  const { busy, run, elabFor } = reprocess
